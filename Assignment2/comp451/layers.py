...
def conv_forward_naive(x, w, b, conv_param):
    ...
    N_padded, C_padded, H_padded, W_padded = x.shape


    for i in range(N):
        image = x_padded[i, :, :, :]
        for filternum in range(F):
            for h_index in range(H_out):
                for w_index in range(W_out):
                    mini_image = image[:, (h_index*stride):(h_index*stride+ HH), (w_index*stride):(w_index*stride+ WW)]
                    out[i, filternum, h_index, w_index] = np.sum(np.multiply(mini_image, w[filternum, :, :, :])) + b[filternum]
...
def max_pool_forward_naive(x, pool_param):
    ...
    H_out = int(1 + (H - h) / stride)
    W_out = int(1 + (W - w) / stride)
    ...
    for i in range(N):       
        for c in range(C):
            ...
            for h_index in range(H_out):
                for w_index in range(W_out):
                    mini_image = image[(h_index*stride):(h_index*stride+ h), (w_index*stride):(w_index*stride+ w)]
                    out[i, c, h_index, w_index] = np.max(mini_image) 
        ...
        for c in range(C):
            ...
            for h_index in range(H_out):
                for w_index in range(W_out):
                    ...
                    max_index = np.unravel_index(np.argmax(mini_image), mini_image.shape)
                    max_index = tuple(map(sum, zip((h_index*stride, w_index*stride), max_index)))
                    dx[(n, c) + max_index] += dout[n, c, h_index, w_index]  

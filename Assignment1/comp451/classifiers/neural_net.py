...
class ThreeLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        loss /= N
        regularization =( np.sum(np.square(W1))  + np.sum(np.square(W2)))*reg
        loss += regularization 
        ...
        dW3 = L2_relu.T.dot(softmax)   # [HxN] * [NxC] = [HxC]
        db3 = softmax.sum(axis=0)
        
        """dW1 = softmax_matrix.dot(W2.T)   # [NxC] * [CxH] = [NxH]
        dfc1 = dW1 * (fc1>0)             # [NxH] . [NxH] = [NxH]
        dW1 = X.T.dot(dfc1)              # [DxN] * [NxH] = [DxH]"""
    
        dW2 = softmax.dot(W3.T)   # [NxC] * [CxH] = [NxH]
        dfc2 = dW2 * (L2>0)             # [NxH] . [NxH] = [NxH]
        #print(dfc2.shape)
        dW2 = L1_relu.T.dot(dfc2)              # [DxN] * [NxH] = [DxH]
        #print(dW2.shape)
        dW2 += reg * 2 * W2
        db2 = dfc2.sum(axis=0)

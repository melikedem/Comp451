...
class KNearestNeighbor(object):
    ...
    def compute_L2_distances_no_loops(self, X):
        ...
        dists = np.sqrt((np.square(X[:, None]-self.X_train[None,:])).sum(axis=2))
    ...
    def compute_L1_distances_one_loop(self, X):
        ...
        for i in range(num_test):
            ...
            #######################################################################
            # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
            dists[i][:]= (np.absolute(X[i]-self.X_train[:])).sum(axis=1)
    ...
    def compute_L1_distances_no_loops(self, X):
        ...
        #########################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        dists[:][:]= (np.absolute(X[:, None]-self.X_train[None,:])).sum(axis=2)
